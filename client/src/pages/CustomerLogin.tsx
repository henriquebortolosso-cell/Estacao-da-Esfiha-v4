...
import { ChefHat, Phone, Lock, Eye, EyeOff, ArrowLeft } from "lucide-react";
...
export default function CustomerLogin() {
  ...
  const login = useMutation({
    mutationFn: () => apiPost("/api/auth/login", { phone: phone.replace(/\D/g,""), password }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/auth/me"] });
      toast({ title: "Bem-vindo de volta!" });
      navigate("/");
    },
    onError: (e: Error) => toast({ title: "Erro ao entrar", description: e.message, variant: "destructive" }),
  });
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="tel" value={phone} onChange={e => setPhone(fmtPhone(e.target.value))}
